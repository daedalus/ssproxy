...
class ProxyHandler:
    ...
    def handle(self) -> None:
        try:
            ...
            target_host = self._extract_target_host(method, path, request_data)
            if (
                self.config.acl.check(self.client_addr[0], target_host)
                == ACLAction.DENY
            ):
                logger.error(
                    f"ACL denied: {self.client_addr[0]} -> {target_host or path}"
                )
    ...
    def _handle_connect(self, path: str) -> None:
        try:
    ...
    def _direct_tunnel(self, host: str, port: int) -> None:
        try:
    ...
    def _tunnel_through_upstream(self, host: str, port: int) -> None:
        ...
        try:
            ...
            if b"200" not in response.split(b"\r\n")[0]:
                logger.error(f"Upstream proxy rejected connection: {response!r}")
                self._send_error(502, "Bad Gateway")
                server_socket.close()
                return
    ...
    def _handle_http_request(self, method: str, path: str, request_data: bytes) -> None:
        ...
        if self.config.logging.show_request_body and body:
            logger.request(f"Body ({len(body)} bytes): {body[:200]!r}")
    ...
    def _extract_host(self, request_line: str) -> str | None:
        parts = request_line.split(" ")
        if len(parts) < 3:
            return None
    ...
    def _extract_target_host(
        self, method: str, path: str, request_data: bytes
    ) -> str | None:
        if method == "CONNECT":
            if ":" in path:
        ...
        headers_end = request_data.find(b"\r\n\r\n")
        headers = (
            request_data[:headers_end].decode("utf-8", errors="ignore")
            if headers_end != -1
            else ""
        )
        return self._extract_host(
            f"{method} {path} HTTP/1.1"
        ) or self._parse_host_from_headers(headers)

    def _forward_direct(
        self, method: str, path: str, headers: str, body: bytes
    ) -> None:
    ...
    def _forward_to_upstream(  # noqa: ARG002
        self, method: str, path: str, request_data: bytes
    ) -> None:
        upstream = self.config.upstream_proxy
    ...
    def _parse_host_from_headers(self, headers: str) -> str | None:
    ...
    def _remove_proxy_headers(self, headers: str) -> str:
        ...
        for line in headers.split("\r\n"):
            ...
            if not any(
                h in lower_line for h in ["proxy-", "connection:", "keep-alive"]
            ):
                result.append(line)
        return "\r\n".join(result)
    ...
    def _relay_response(  # noqa: ARG002
        self, server_socket: socket.socket, host: str, port: int
    ) -> None:
