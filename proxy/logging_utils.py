...
class ColoredLogger:
    def __init__(self, enabled: bool = True, colorful: bool = True):
    ...
    def _print(self, level: LogLevel, message: str, **kwargs):
        if not self.enabled:
            return
    ...
    def log_request_line(self, method: str, url: str, client_addr: str):
        if not self.enabled:
            return
        if self.colorful:
            print(
    ...
    def log_response_line(self, status_code: int, reason: str, client_addr: str):
        if not self.enabled:
            return
        if status_code < 300:
            color = Color.GREEN
    ...
    def log_connect(self, host: str, port: int, client_addr: str):
        if not self.enabled:
            return
        if self.colorful:
            print(
    ...
    def log_connect_success(self, host: str, port: int, client_addr: str):
        if not self.enabled:
            return
        if self.colorful:
            print(
